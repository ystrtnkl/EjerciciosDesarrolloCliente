...
const useSupabase = () => {
    ...
    //Obtiene todas las listas (solo las del usuario con el que esté iniciado sesión).
    const obtenerListas = async () => {
        ...
        try {
            ...
            setErrorSupabase(e?.message || "No se ha podido obtener las listas");
        } finally {
            setCargandoSupabase(false);
        }
    }

    //Crea una nueva lista asignada al usuario iniciado sesión.
    const crearLista = async (nombre, descripcion) => {
        setErrorSupabase("");
        setCargandoSupabase(true);
        try {

        } catch (e) {
            setErrorSupabase(e?.message || "No se ha podido crear la lista");
        } finally {
            setCargandoSupabase(false);
        }
    }

    //Actualiza los datos de una lista (si tiene permisos).
    const actualizarLista = async (nombre, descripcion) => {
        setErrorSupabase("");
        setCargandoSupabase(true);
        try {

        } catch (e) {
            setErrorSupabase(e?.message || "No se ha podido editar la lista");
        } finally {
            setCargandoSupabase(false);
        }
    }

    //Borra una lista (si tiene permisos).
    const borrarLista = async (uuid) => {
        ...
        try {
            ...
            setErrorSupabase(e?.message || "No se ha podido borrar la lista");
        } finally {
            setCargandoSupabase(false);
        }
    }

    //Añade/quita un producto de una lista, el número indica la cantidad alterativa.
    const cambiarProductoLista = async (uuidLista, uuidProducto, cantidad) => {
        setErrorSupabase("");
        setCargandoSupabase(true);
        try {
    ...
    return { obtenerProductos, cargandoSupabase, errorSupabase, insertarProducto, editarProducto, borrarProducto, obtenerListas, crearLista, actualizarLista, borrarLista, cambiarProductoLista };
};
