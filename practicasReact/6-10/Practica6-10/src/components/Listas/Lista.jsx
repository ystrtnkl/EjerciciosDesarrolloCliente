...
const Lista = (props) => {
  ...
  //Valida antes de enviar los datos de la lista a guardar/editar.
  const validar = () => {
  ...
  //Manda a guardar/editar los datos de la lista.
  const guardar = async () => {
    if (validar()) {
  ...
  //Handler para los botones de añadir/quitar.
    ...
    <>
      {errorLista ? (<CajaError texto={errorLista} />) : ((cargandoLista || typeof listaActual === "undefined") ? (<Cargando />) : (<div>
        ...
        <form onChange={(e) => { manejadorInput(e, setListaActual, listaActual) }}>
          ...
          <InputBasico nombre="descripcion" titulo="Descripción:" tipo="textarea" valor={listaActual.descripcion} validador={validarDescripcionLista} mensajeError="La descripción de la lista debe tener menos de 512 carácteres" />
        </form>
