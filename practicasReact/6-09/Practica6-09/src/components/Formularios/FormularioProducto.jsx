...
function FormularioProducto(props) {
  ...
  const enviar = async (e) => {
    ...
    if (validar()) {
        console.log({...datosProducto, peso: parseFloat(datosProducto.peso), precio: parseFloat(datosProducto.precio), duegno: ""})
      if (props.editar) {
  ...
  return (
    <>
      <form onChange={(e) => { manejadorInput(e, setDatosProducto, datosProducto) }}>
        ...
        <InputBasico nombre="url_imagen" titulo="URL de la imágen:" tipo="text" valor={datosProducto.url_imagen} validador={validarUrl} mensajeError="Debes poner una URL válida (http/https, hasta 128 carácteres)" />
