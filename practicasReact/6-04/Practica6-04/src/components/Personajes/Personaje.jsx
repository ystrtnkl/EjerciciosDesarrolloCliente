...
import { idPorUrl, formatearGeneroEspagnol, formatearMedida } from '../../libraries/formateos.js';

//Componente para mostrar un personaje y las películas en las que sale (solo con el atributo "expandido" en true).
function Personaje(props) {

  const generoFormateado = formatearGeneroEspagnol(props.personaje.gender); //Formatear el atributo género en español.
  ...
  const url = props.personaje.url ? idPorUrl(props.personaje.url) : "";
    ...
    <div className={(props.expandir ? "personaje-expandido" : "") + " personaje"}>
      {props.personaje.fallo
        ...
        : (<div>
          ...
          <p>Altura: {formatearMedida(props.personaje.height, "centimeter") ?? "Desconocidos"}</p>
          <p>Masa: {formatearMedida(props.personaje.mass, "kilogram") ?? "Desconocidos"}</p>
          {/*Para traducir el color habría que tener un diccionario de cada color en español e inglés, pero teniendo en cuenta la cantidad de colores posibles que hay es mejor dejarlo en inglés de momento.*/}
