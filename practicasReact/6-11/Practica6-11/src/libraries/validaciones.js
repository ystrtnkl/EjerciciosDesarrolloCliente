...
//Valida la descripción de un perfil de un usuario, que debe tener menos de 512 carácteres.
const validarDescripcionPerfil = (descripcion) => {
    return typeof descripcion === "string" && descripcion.length < 512;
}

//Valida entero el perfil de un usuario con todos sus campos
const validarPerfil = (datos) => {
    return typeof datos === "object"
        && validarNombreUsuario(datos.nombre_completo)
        && validarDescripcionPerfil(datos.descripcion)
        && validarUrl(datos.avatar);
}

export { validarContrasegna, validarDescripcionPerfil, validarPerfil, validarDatosLista, validarDescripcionLista, validarNombreLista, validarCorreo, validarNombreUsuario, validarUuid, validarDescripcionProducto, validarNombreProducto, validarPesoProducto, validarPrecioProducto, validarUrl, validarDatosProducto, validarDuegno }